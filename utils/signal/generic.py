...
def butter_lowpass_filter(data, cutoff, fs, order=5):
    ...
    normal_cutoff = (
        float(cutoff) / nyq_freq
    )  # normalized cutoff for the butter function
    b, a = scipy.signal.butter(
        N=order, Wn=normal_cutoff, btype="lowpass", analog=False, output="ba"
    )
...
def get_data_fit(x, y, function):
    ...
    def fit_inverse(_x, _y):
        ...
        if len(_x) != len(_y):
            raise ValueError(
                "Cannot fit the curve because x and " "y do not have the same size !"
            )
    ...
    def fit_first_order(_x, _y):
        ...
        popt, pcov = scipy.optimize.curve_fit(
            func, xdata=_x, ydata=_y, bounds=((0, 0.01), (numpy.inf, numpy.inf))
        )
        fit_func_call = lambda s: func(s, popt[0], popt[1])
        fit_func_str = "f(x) = {:.0f} * (1 - exp(-x / {:.0f}))".format(popt[0], popt[1])
        return fit_func_call, fit_func_str, popt
    ...
    if y_range == 0:
        ...
        fit_error = y - f_y
        nrmsd = numpy.sqrt(numpy.sum(numpy.square(fit_error)) / len(y)) / y_range
